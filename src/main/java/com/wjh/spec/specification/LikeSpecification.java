...
public class LikeSpecification<T> extends AbstractSpecification<T> {
    ...
    public LikeSpecification(String property, Object... patterns) {
        this.property = property;
        this.patterns = patterns;
    }

    public LikeSpecification(Getter<T, ?> getter, Object... patterns) {
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (patterns.length == 1) {
            return cb.like(from.get(field), patterns[0].toString());
        }
        Predicate[] predicates = new Predicate[patterns.length];
        for (int i = 0; i < patterns.length; i++) {
            predicates[i] = cb.like(from.get(field), patterns[i].toString());
        }
